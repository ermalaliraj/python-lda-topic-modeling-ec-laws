...
Documents are download from the Official Journal of European Commission using this [project](https://github.com/ermalaliraj/eur-lex-official-journal-sparql).

### Project Structure
The project is split in the following files: 
1. `serialize_documents_array.py`
    - Loads all documents, reads fileContent, cleans xml, leammatize, and creates the array `documents` with rows [fileName, fileContent] for each document.
...
2. `serialize_model.py`
    ...
    - OUTPUT: serializes in filesystem:
        ...
        - `lda_model`, the model.
    - EXAMPLES: different model and the related bow_corpus for regulations of the year 2016, 2017, 2018, 2019 2020 and ALL are created in the folder `./model`. 
3. `visualize.py`
...
4. `predict.py`
    - Tests the model using an unseen phrases and print the predicted Topic closer to the phrase. <br/>
    See [`predict.out`](predict.out) with all prediction.
    - OUTPUT: Print in the console the result of 4 predictions.
5. `serialize_mapper_topic_to_docs.py`
    - Builds a mapper Topic-Documents expressed through a list as follow. 
        1) For each document, uses its content to predict the related topic.
